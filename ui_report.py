...
class Report():
    ...
    def startReporting(self, controller):
        ...
        self.button_gen = tk.Button(report_frame_bot, text="Save", font=self.FONT, width=10, command=lambda: self.createReport())
    ...
    def createReport(self, event=None):
        ...
        if (self.input_name == "") == False:
            self.existing = self.database.checkExistance('transformed', self.input_name)
            self.save_taken = self.database.checkExistance('report', self.output_name)
